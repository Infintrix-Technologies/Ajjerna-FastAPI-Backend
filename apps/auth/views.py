...
from .schema import RefreshToken, UserReponseSchema, RegisterRequestSchema, RegisterRequestSchema
...
def refresh_token(request: RefreshToken):
    ...
    if payload is None or payload == {}:
        ...
        raise HTTPException(status_code=401, detail="Please send valid payload in body", headers={
                            "WWW-Authenticate": "Bearer"},)

    if payload and payload.get("type") != "refresh":
        raise HTTPException(status_code=400, detail="Please send valid refresh token", headers={
                            "WWW-Authenticate": "Bearer"},)

    user = User.find(email=payload["email"])

    if not user:
        raise HTTPException(status_code=400, detail="Please send valid refresh token for valid user", headers={
                            "WWW-Authenticate": "Bearer"},)
    is_refresh_expired = datetime.now() >= datetime.utcfromtimestamp(payload.get("exp"))

    if is_refresh_expired:
        raise HTTPException(status_code=401, detail="Refresh token expired please login again", headers={
                            "WWW-Authenticate": "Bearer"},)
