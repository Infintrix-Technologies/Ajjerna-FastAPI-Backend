...
from apps.auth.schema import UserReponseSchema, RegisterRequestSchema, RegisterRequestSchema
...
async def create_new_user(request : RegisterRequestSchema, db: Session = Depends(get_db)):
...
async def update_user(id,request : RegisterRequestSchema, db: Session = Depends(get_db)):
    try:
